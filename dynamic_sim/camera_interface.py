...
class CameraInterface():
    def __init__(self):
        ...
        self.cTurret = CameraTurret(CameraTurret([Target(np.array([0, 100, 100]).reshape(3, 1), "t0")]))
    ...
    def getCState(self):
        try:
            self.cameraPath = self.Comms.get('cState').payload
        except queue.Empty:
            pass
    ...
    def run(self):
        ...
        while(True):
            ...
            for t_link, target in zip(t_links, targets):
                ...
                targ_pos = self.cTurret.representTarget(self, t_link) + self.cTurret.orig
